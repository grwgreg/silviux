#Silviux modified to support changing decoders, adaption state is now saved in /tmp
__author__ = 'tanel'
...
class ServerWebsocket(WebSocketClient):
    ...
    def __init__(self, uri, decoder_pipeline, post_processor, full_post_processor=None, default_decoder=None):
        self.uri = uri
        self.decoder_pipeline = decoder_pipeline
        self.post_processor = post_processor
        self.full_post_processor = full_post_processor
        self.decoder_name = default_decoder
            ...
            if self.state != self.STATE_CANCELLING and self.state != self.STATE_EOS_RECEIVED and self.state != self.STATE_FINISHED:
                if isinstance(m, ws4py.messaging.BinaryMessage):
                    ...
                    if 'adaptation_state' in props:
                        logger.warn("%s: Recieved adaptation state message but state is now saved on server" % (self.request_id))
                    ...
                    elif 'change_decoder' in props and self.decoder_name != props['change_decoder']:
                        self.decoder_name = props['change_decoder']
                        logger.info("%s: Resetting decoder pipeline to use decoder key %s" % (self.request_id, self.decoder_name))
                        self.decoder_pipeline.reset_pipeline(self.decoder_name)
                        self.decoder_pipeline.set_result_handler(self._on_result)
                        self.decoder_pipeline.set_full_result_handler(self._on_full_result)
                        self.decoder_pipeline.set_error_handler(self._on_error)
                        self.decoder_pipeline.set_eos_handler(self._on_eos)
    ...
    def _on_eos(self, data=None):
        self.last_decoder_message = time.time()
        self.state = self.STATE_FINISHED
        # silviux changed to save adaptation state on server instead of sending to client
    ...
    # silviux removed, now we save on server see save_adaptation_state in decoder2.py
...
def main():
    ...
    if USE_NNET2:
        default_decoder = conf["default-decoder"]
        conf["decoder"] = conf["decoders"][default_decoder]
        decoder_pipeline = DecoderPipeline2(conf)
    else:
        #other decoders will work as single use but the code for switching between decoders was never added
        raise Exception('This silviux server was modified to only support nnet2 and nnet3 decoders')
    ...
    while True:
        ws = ServerWebsocket(args.uri, decoder_pipeline, post_processor, full_post_processor=full_post_processor, default_decoder=default_decoder)
        try:
